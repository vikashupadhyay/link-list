...
int asArray(LinkedList list, void **arr, int maxElements){
	Element *ele = list.head;
	int counter =0;
	while(ele!=NULL && maxElements>counter){
		arr[counter] = ele->value;
		ele = ele->next;
		counter++;
	}
	return counter;
...
LinkedList reverse(LinkedList list){
	...
	for (int i = list.length-1; i >=0; i--){
		add_to_list(&newList,getElementAt(list,i));
	}
	return newList;
};
...
LinkedList map(LinkedList list, ConvertFunc *match, void *hint){
	...
	void *modifiedValue;
	while(ele!=NULL){
		modifiedValue = malloc(sizeof(void *));
		match(&hint,ele->value,modifiedValue);
		ele = ele->next;
		add_to_list(&newList,modifiedValue);
	}
	return newList;
};
